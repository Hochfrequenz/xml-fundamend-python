"""
This module contains the SQLModel class for the AHB formatversion diff view and a function to create it.
The view allows comparing the SAME Pruefidentifikator between two AHB format versions to find rows that
were added, deleted, or modified.
For comparing two DIFFERENT Pruefidentifikatoren within the SAME format version, see ahb_pruefi_diff_view.py.
"""
    ...
    try:
        ...
        if count == 0:
            _logger.warning(
                ...
                "The v_ahb_formatversion_diff view will not return any results. "
                "Make sure to call create_ahbtabellen_view() after populating the database."
            )
    except sqlalchemy.exc.OperationalError:
        _logger.warning(
            "v_ahbtabellen does not exist. "
            "The v_ahb_formatversion_diff view requires v_ahbtabellen to be created first. "
            "Call create_ahbtabellen_view() before create_ahb_formatversion_diff_view()."
        )


def create_ahb_formatversion_diff_view(session: Session) -> None:
    """
    Create a view for comparing the same Pruefidentifikator across two AHB format versions.
    ...
    _execute_bare_sql(
        session=session, path_to_sql_commands=Path(__file__).parent / "create_ahb_formatversion_diff_view.sql"
    )
    _logger.info("Created view %s", AhbFormatversionDiffLine.__tablename__)


class AhbFormatversionDiffLine(SQLModel, table=True):
    """
    Model that represents the diff view for comparing the same Pruefidentifikator across AHB format versions.
    ...
    __tablename__ = "v_ahb_formatversion_diff"
